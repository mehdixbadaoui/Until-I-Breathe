...
public class PlayEventSounds : MonoBehaviour
{
    ...
    public void RTPCGameObjectValue(Vector3 distUniFromObject, float maxDistance, GameObject gameObject, string nameOfEvent, string nameOfRTPC = "", float coeffSpeed = 1)
    {
        ...
        if (Mathf.Abs(distUniFromObject.z) <= maxDistance || Mathf.Abs(distUniFromObject.y) <= maxDistance || Mathf.Abs(distUniFromObject.x) <= maxDistance)
        {
            ...
            float volumeX = (100 - Mathf.Abs(distUniFromObject.x * 100f / maxDistance)) * coeffSpeed;
    ...
    public void RTPCGameObjectValueEnterRoomLvl3_3(float distUniFromObjectLeft, float distUniFromObjectRight, float maxDistance, GameObject gameObject, float coeffAttenuation)
    {
        ...
        AkSoundEngine.SetRTPCValue("VentVolume", musicVolume);
